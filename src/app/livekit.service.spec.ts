...
describe('LivekitService', () => {
  ...
  beforeEach(() => {
    ...
    roomMock = jasmine.createSpyObj(
      'Room',
      ['on', 'connect', 'emit', 'disconnect'],
      {
    ...
    spyOn(service, 'updateParticipantNames').and.callFake(() => {});
    spyOn(service, 'remoteParticipantAfterLocal').and.callFake(() => {});
  });
  ...
  describe('LiveKitService: connectToRoom', () => {
    it('should connect to the room and dispatch the room name', async () => {
      const wsURL = 'ws://test-url';
      const token = 'test-token';

      // Mock room.connect to resolve successfully
      roomMock.connect.and.callFake(async () => {
        Object.defineProperty(roomMock, 'name', {
          value: 'Test Room',
          writable: false,
        });
      });
      ...
      // Verify room.connect is called with the correct arguments
      expect(roomMock.connect).toHaveBeenCalledWith(wsURL, token);

      // Verify that updateParticipantNames and remoteParticipantAfterLocal are called
      expect(service.updateParticipantNames).toHaveBeenCalled();
      expect(service.remoteParticipantAfterLocal).toHaveBeenCalled();

      // Verify the room name dispatch action
    ...
    it('should throw an error if room.connect fails', async () => {
      const wsURL = 'ws://test-url';
      const token = 'test-token';

      // Mock room.connect to reject with an error
      roomMock.connect.and.returnValue(Promise.reject('Connection Error'));

      await expectAsync(service.connectToRoom(wsURL, token)).toBeRejectedWith(
        'Connection Error'
      );
      ...
      // Ensure updateParticipantNames and remoteParticipantAfterLocal are not called
      expect(service.updateParticipantNames).not.toHaveBeenCalled();
      expect(service.remoteParticipantAfterLocal).not.toHaveBeenCalled();

      // Verify the room name dispatch action is not called
      expect(storeMock.dispatch).not.toHaveBeenCalled();
    });
  });

  it('should return the room name and log it', () => {
    // Mock the room name
    Object.defineProperty(roomMock, 'name', {
      value: 'Test Room',
      writable: false,
    ...
    // Call the method
    const roomName = service.getRoomName();

    // Verify that the room name is assigned to roomNameFromUrl
    expect(service.roomNameFromUrl).toEqual('Test Room');

    // Verify that the method returns the correct room name
    expect(roomName).toEqual('Test Room');
  });
  ...
  describe('disconnectRoom', () => {
    it('should call disconnect and emit next and complete when room is available', fakeAsync(() => {
      // Assign the mock room
      service.room = roomMock;
      ...
      // Verify the room's disconnect method is called
      expect(roomMock.disconnect).toHaveBeenCalled();

      // Verify that next and complete are called
      expect(nextSpy).toHaveBeenCalled();
      expect(completeSpy).toHaveBeenCalled();
    }));

    it('should only emit complete when room is not available', fakeAsync(() => {
      // Set the room to null to simulate no room being available
      service.room = null;
      ...
      // Verify the room's disconnect method is not called
      expect(roomMock.disconnect).not.toHaveBeenCalled();

      // Verify that next is not called and complete is called
  ...
  describe('async breakoutRoomAlert', () => {
    it('should update breakoutRoomsData and emit updated data', async () => {
      // Arrange
      const participants = ['participant1', 'participant2'];
      const roomName = 'Room1';

      spyOn(service, 'publishBreakoutRoom').and.returnValue(Promise.resolve());
      const breakoutRoomsDataUpdatedSpy = spyOn(
        service.breakoutRoomsDataUpdated,
        'next'
      );

      // Set initial breakout room data
      ...
      expect(service.breakoutRoomsData).toEqual([
        {
          roomName: 'Room1',
          participantIds: ['participant1', 'participant2'], // Ensure no duplicates
          type: 'breakoutRoom',
        },
      ]);

      expect(breakoutRoomsDataUpdatedSpy).toHaveBeenCalledWith(
        service.breakoutRoomsData
      );

      console.log('Breakout room data:', service.breakoutRoomsData);
    });

    it('should add a new room if it does not exist', async () => {
      // Arrange
      const participants = ['participant3'];
  ...
  describe('async publishBreakoutRoom', () => {
    it('should publish data with correct parameters', async () => {
      // Arrange
      const message = {
        type: 'breakoutRoom',
      ...
      const recipientIds = ['participant1', 'participant2'];

      // Mock the localParticipant's publishData method
      const publishDataSpy = jasmine.createSpy('publishData');
      const roomMock = jasmine.createSpyObj('Room', [], {
        localParticipant: {
          publishData: publishDataSpy,
        },
      });

      service.room = roomMock as any; // Assign the mock room to the service

      // Act
      await service.publishBreakoutRoom(message, recipientIds);

      // Assert
      expect(publishDataSpy).toHaveBeenCalledOnceWith(
        jasmine.any(Uint8Array), // The first argument should be a Uint8Array
        {
          reliable: true,
          destinationIdentities: recipientIds,
        }
      );

      // Check if the message was correctly encoded to Uint8Array
      const expectedData = new TextEncoder().encode(JSON.stringify(message));
  ...
  //   it('should handle ActiveSpeakersChanged event and update active speakers', () => {
  //     // Arrange: Mock the room and the event
  //     const speakerMock1 = jasmine.createSpyObj('Participant', [
  //       'identity',
  //       'audioLevel',
  //     ]);
  //     const speakerMock2 = jasmine.createSpyObj('Participant', [
  //       'identity',
  //       'audioLevel',
  //     ]);

  //     // Set mock properties for speakers
  //     speakerMock1.identity = 'Speaker1';
  //     speakerMock1.audioLevel = 0.8;
  //     speakerMock2.identity = 'Speaker2';
  //     speakerMock2.audioLevel = 0.5;

  //     // Create the mock room object with an on method
  //     const roomMock = jasmine.createSpyObj('Room', ['on']);

  //     // Create the service instance and assign the roomMock to it
  //     service.room = roomMock as any;

  //     // Spy on the updateActiveSpeakerBorders method
  //     const updateActiveSpeakerBordersSpy = spyOn(
  //       service,
  //       'updateActiveSpeakerBorders'
  //     ).and.stub();

  //     // Spy on the console.log to capture the logged output
  //     const consoleLogSpy = spyOn(console, 'log');

  //     // Spy on createSpeakerAvatar (even if it's commented out, ensure it's covered)
  //     const createSpeakerAvatarSpy = spyOn(
  //       service,
  //       'createSpeakerAvatar'
  //     ).and.stub();

  //     // Act: Trigger the ActiveSpeakersChanged event
  //     const speakers = [speakerMock1, speakerMock2];
  //     roomMock.on(
  //       RoomEvent.ActiveSpeakersChanged,
  //       (speakers: Participant[]) => {
  //         console.log('Active speakers:', speakers);
  //         service.activeSpeakers = speakers;
  //         service.updateActiveSpeakerBorders();

  //         // Simulating avatar creation (uncommented in actual code)
  //         speakers.forEach((speaker) => {
  //           // If the createSpeakerAvatar function is uncommented, we expect it to be called.
  //           createSpeakerAvatarSpy(speaker);
  //           console.log(
  ...
  //     // Trigger the event manually (simulate the event call)
  //     roomMock.on.calls.mostRecent().args[1](speakers);

  //     // Assert: Check that activeSpeakers was updated
  //     expect(service.activeSpeakers).toEqual(speakers);

  //     // Assert: Check that updateActiveSpeakerBorders was called
  //     expect(updateActiveSpeakerBordersSpy).toHaveBeenCalled();

  //     // Assert: Check the console logs for correct speaker information
  //     expect(consoleLogSpy).toHaveBeenCalledWith('Active speakers:', speakers);
  //     expect(consoleLogSpy).toHaveBeenCalledWith(
  //       'Speaker1 is speaking with audio level 0.8'
  //     );
  //     expect(consoleLogSpy).toHaveBeenCalledWith(
  //       'Speaker2 is speaking with audio level 0.5'
  //     );

  //     // Assert: Check that createSpeakerAvatar was called for each speaker
  //     expect(createSpeakerAvatarSpy).toHaveBeenCalledWith(speakerMock1);
  //     expect(createSpeakerAvatarSpy).toHaveBeenCalledWith(speakerMock2);
  //   });
  // });

  describe('updateActiveSpeakerBorders', () => {
    let roomMock: any;
    let participantMock1: any;
    let participantMock2: any;

    beforeEach(() => {
      // Mock the participants and room
      participantMock1 = {
        sid: 'participant1',
        audioLevel: 0.8,
      ...
      roomMock = {
        localParticipant: participantMock1,
        remoteParticipants: new Map([[participantMock2.sid, participantMock2]]),
      };

      // Create the service instance
      service.room = roomMock;
      service.activeSpeakers = [participantMock1]; // Mock active speakers
    });
    it('should remove the border for inactive speakers or speakers with zero audio level', () => {
      // Arrange: Mock DOM element for inactive speaker
      const mockElement = {
        style: {
          transition: '',
          border: '',
      ...
      // Assert: Ensure no border is applied to the inactive speaker
      expect(mockElement.style.transition).toBe('');
      expect(mockElement.style.border).toBe('');
    });

    it('should call createSpeakerAvatar for each participant', () => {
      // Arrange: Spy on createSpeakerAvatar
      const createSpeakerAvatarSpy = spyOn(
        service,
        'createSpeakerAvatar'
      ).and.stub();
      ...
      // Assert: Ensure createSpeakerAvatar was called for each participant
      expect(createSpeakerAvatarSpy).toHaveBeenCalledWith(participantMock1);
      expect(createSpeakerAvatarSpy).toHaveBeenCalledWith(participantMock2);
    });

    it('should update the active speaker borders based on active speakers', () => {
      // Arrange: Mock DOM elements
      const mockElement1 = {
        style: {
          transition: '',
          border: '',
      ...
      spyOn(document, 'getElementById')
        .withArgs(participantMock1.sid)
        .and.returnValue(mockElement1 as any)
        .withArgs(participantMock2.sid)
        .and.returnValue(mockElement2 as any);

      // Act: Call the method with an active speaker list
      service.activeSpeakers = [participantMock1]; // Only participantMock1 is active
      service.updateActiveSpeakerBorders();

      // Assert: Check if the active participant has a border and the inactive does not
      expect(mockElement1.style.transition).toBe('border 0.3s ease-in-out');
      expect(mockElement1.style.border).toBe('4px solid #28a745');
      expect(mockElement2.style.transition).toBe('');
      expect(mockElement2.style.border).toBe('');
    });
  });

  describe('showInitialSpeaker', () => {
    let participantMock1: any;
    let consoleWarnSpy: jasmine.Spy;
    let createSpeakerAvatarSpy: jasmine.Spy;

    beforeEach(() => {
      // Mock the remote participants and room
      participantMock1 = {
        sid: 'participant1',
        audioLevel: 0.8,
        identity: 'Participant 1',
      ...
      roomMock = {
        remoteParticipants: new Map([[participantMock1.sid, participantMock1]]),
      } as any;

      // Create the service instance
      service.room = roomMock;
      service.activeSpeakers = []; // Initial empty list of active speakers

      // Spy on console.warn to capture warnings
      consoleWarnSpy = spyOn(console, 'warn');

      // Spy on createSpeakerAvatar to check if it's called
      createSpeakerAvatarSpy = spyOn(service, 'createSpeakerAvatar').and.stub();
    });

    it('should set the first remote participant as the initial speaker', (done) => {
      // Act: Call the method
      service.showInitialSpeaker();

      // Assert: Check if the first remote participant is added to activeSpeakers
      expect(service.activeSpeakers).toContain(participantMock1);

      // Ensure createSpeakerAvatar is called after the timeout
      setTimeout(() => {
        expect(createSpeakerAvatarSpy).toHaveBeenCalledWith(participantMock1);
        done(); // Signal to Jasmine that the async test is done
      }, 0);
    });

    it('should log a warning when no remote participants are found', () => {
      // Arrange: Mock empty remote participants map
      roomMock.remoteParticipants = new Map();

      // Act: Call the method
      service.showInitialSpeaker();

      // Assert: Check that a warning is logged
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'No remote participants found.'
      );
    });

    it('should not call createSpeakerAvatar if there are no remote participants', () => {
      // Arrange: Mock empty remote participants map
      roomMock.remoteParticipants = new Map();
      ...
      // Assert: Check that createSpeakerAvatar is not called
      expect(createSpeakerAvatarSpy).not.toHaveBeenCalled();
    });
  });

  describe('sendCloseAlertToBreakoutRooms', () => {
    ...
    beforeEach(() => {
      ...
      meetingServiceMock = {
        sendCloseBreakoutRoomAlert: jasmine
          .createSpy('sendCloseBreakoutRoomAlert')
          .and.returnValue({
            subscribe: jasmine
              .createSpy('subscribe')
              .and.callFake((observer: any) => {
                observer.next(); // Simulate success
              }),
          }),
      };

      // Create the service instance with the mock
      service.meetingService = meetingServiceMock;
      service.breakoutRoomsData = [
        { roomName: 'Room 1' },
        { roomName: 'Room 2' },
      ];
    ...
    it('should send close alert to breakout rooms when rooms are available', () => {
      // Act: Call the method
      service.sendCloseAlertToBreakoutRooms();

      // Assert: Check that the sendCloseBreakoutRoomAlert method is called for each room
      expect(
        meetingServiceMock.sendCloseBreakoutRoomAlert
      ).toHaveBeenCalledTimes(2);
      expect(
        meetingServiceMock.sendCloseBreakoutRoomAlert
      ).toHaveBeenCalledWith(
        'Room 1',
        '{"type":"closeRoomAlert","countdown":60}'
      );
      expect(
        meetingServiceMock.sendCloseBreakoutRoomAlert
      ).toHaveBeenCalledWith(
      ...
      // Assert: Check that success log was called for each room
      expect(consoleLogSpy).toHaveBeenCalledWith(
        'Close alert sent successfully to breakout room: Room 1'
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        'Close alert sent successfully to breakout room: Room 2'
      );
    });

    it('should log a warning if no breakout rooms are available', () => {
      // Arrange: Set breakoutRoomsData to an empty array
      service.breakoutRoomsData = [];

      // Act: Call the method
      service.sendCloseAlertToBreakoutRooms();

      // Assert: Ensure that a warning is logged
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'No breakout rooms available to send close alert.'
      );
    });

    it('should log an error message if sending the close alert fails', () => {
      // Arrange: Modify the mock to simulate an error
      meetingServiceMock.sendCloseBreakoutRoomAlert.and.returnValue({
        subscribe: jasmine
          .createSpy('subscribe')
          .and.callFake((observer: any) => {
            observer.error(new Error('Failed to send alert'));
          }),
      });
      ...
      // Assert: Ensure that an error message is logged
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Failed to send close alert to breakout room: Room 1',
        new Error('Failed to send alert')
      );
    });
  });

  describe('ActiveSpeakersChanged Event Handler', () => {
    let roomMock: any;
    let consoleLogSpy: jasmine.Spy;

    beforeEach(() => {
      // Mocking the room and the event handling
      roomMock = {
        on: jasmine.createSpy('on'),
      };

      // Create the service instance with the mock room
      service.room = roomMock;
      service.activeSpeakers = [];

      // Spying on console.log to track log messages
      consoleLogSpy = spyOn(console, 'log');
    });

    it('should update active speakers and call updateActiveSpeakerBorders when ActiveSpeakersChanged event is triggered', () => {
      // Arrange: Create a mock list of speakers
      const mockSpeakers = [
        { identity: 'Speaker 1', audioLevel: 5, sid: 'sid1' },
        { identity: 'Speaker 2', audioLevel: 3, sid: 'sid2' },
      ] as any;

      // Spy on the updateActiveSpeakerBorders method
      spyOn(service, 'updateActiveSpeakerBorders');

      // Act: Simulate triggering the ActiveSpeakersChanged event
      roomMock.on.calls.mostRecent().args[1](mockSpeakers); // Call the callback directly

      // Assert: Check that activeSpeakers are updated correctly
      expect(service.activeSpeakers).toEqual(mockSpeakers);

      // Assert: Check that updateActiveSpeakerBorders was called
      expect(service.updateActiveSpeakerBorders).toHaveBeenCalled();

      // Assert: Check that log messages are generated for each speaker
      expect(consoleLogSpy).toHaveBeenCalledWith(
        'Active speakers:',
        mockSpeakers
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        'Speaker 1 is speaking with audio level 5'
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        'Speaker 2 is speaking with audio level 3'
      );
    });
  });
});
